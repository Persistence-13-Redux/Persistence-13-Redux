/datum/PersistentDB
	var/datum/db_records/CrewRecords/crew_records = new()

/datum/db_records/CrewRecords
	var/const/CREW_RECORDS_TABLE = "crew_records"

//=========================
// Records
//=========================
/datum/db_records/CrewRecords/proc/CreateGlobalCrewRecord(var/datum/computer_file/report/crew_record/records)
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("INSERT INTO [CREW_RECORDS_TABLE] VALUES [records.to_sql()];")
	dbq.Execute()
	if(dbq.RowsAffected())
		return TRUE

/datum/db_records/CrewRecords/proc/RemoveGlobalCrewRecord(var/real_name)
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("DELETE FROM [CREW_RECORDS_TABLE] WHERE name = [SQL_HELPER.Quote(real_name)];")
	dbq.Execute()
	if(dbq.RowsAffected())
		return TRUE

/datum/db_records/CrewRecords/proc/GetGlobalCrewRecord(var/real_name)
	var/list/results = GetGlobalCrewRecords("name = [SQL_HELPER.Quote(real_name)]")
	return LAZYLEN(results)? results[1] : null

/datum/db_records/CrewRecords/proc/GetGlobalCrewRecords(var/condition)
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("SELECT * FROM [CREW_RECORDS_TABLE] WHERE [condition];")
	dbq.Execute()
...
/datum/db_records/CrewRecords/proc/UpdateGlobalCrewRecord(var/datum/computer_file/report/crew_record/records)
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("SELECT * FROM [CREW_RECORDS_TABLE] WHERE name = [SQL_HELPER.Quote(records.get_name())];")
	dbq.Execute()
	if(dbq.NextRow())
		records.parse_row(dbq.GetRowData())
		return records

/datum/db_records/CrewRecords/proc/GetGlobalCrewRecords_CKEY(var/ckey)
	return GetGlobalCrewRecords("ckey = [SQL_HELPER.Quote(ckey)]")

/datum/db_records/CrewRecords/proc/CommitGlobalCrewRecord(var/datum/computer_file/report/crew_record/records)
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("UPDATE [CREW_RECORDS_TABLE] SET ([records.to_sql()]) WHERE name = [SQL_HELPER.Quote(records.get_name())];")
	dbq.Execute()
	return dbq.RowsAffected() > 0

/datum/db_records/CrewRecords/proc/CommitFieldValue(var/real_name, var/datum/report_field/field)
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("UPDATE [CREW_RECORDS_TABLE] SET ([field.to_sql()]) WHERE name = [SQL_HELPER.Quote(real_name)];")
	dbq.Execute()
	return dbq.RowsAffected() > 0
...
/datum/report_field/proc/to_sql()
	return text("[] = []", name, SQL_HELPER.Quote(value))