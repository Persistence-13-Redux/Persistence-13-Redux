/datum/PersistentDB
	var/datum/db_records/Contracts/contracts = new()

/datum/db_records/Contracts
...
*/
/datum/db_records/Contracts/proc/CreateContract(var/obj/item/weapon/paper/contract/C)
	if(!C) return
	if(!PSDB.check_connection()) return
	var/uuid = null
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("INSERT INTO [CONTRACTS_TABLE] VALUES [C.to_sql()];")
	dbq.Execute()
	if(dbq.RowsAffected())
		var/DBQuery/idquery = PSDB.psdbcon.NewQuery("SELECT LAST_INSERT_ID();")
		idquery.Execute()
		uuid = idquery.item[1]
	return uuid
...
*/
/datum/db_records/Contracts/proc/RemoveContract(var/contract_id)
	if(!isnum(contract_id)) return
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("DELETE FROM [CONTRACTS_TABLE] WHERE uuid = [contract_id];")
	dbq.Execute()
...
*/
/datum/db_records/Contracts/proc/GetContract(var/contract_id)
...
*/
/datum/db_records/Contracts/proc/GetContracts_SignedBy(var/signed_by)
...
*/
/datum/db_records/Contracts/proc/GetContracts_PaidTo(var/paid_to)
	return __GetContracts("paid_to = [paid_to]")

/*
	Used internally to simplify making contract queries. Not intended to be used outside.
*/
/datum/db_records/Contracts/proc/__GetContracts(var/condition)
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("SELECT * FROM [CONTRACTS_TABLE] WHERE [condition];")
	dbq.Execute()
...
*/
/datum/db_records/Contracts/proc/SetContract(var/obj/item/weapon/paper/contract/C)
	if(!C) return
	if(!PSDB.check_connection()) return
	var/DBQuery/dbq = PSDB.psdbcon.NewQuery("UPDATE [CONTRACTS_TABLE] SET ([C.to_sql()]) WHERE uuid = [C.uuid];")
	dbq.Execute()
	return dbq.RowsAffected()
...
/obj/item/weapon/paper/contract/proc/to_sql()
	. =  "uuid =              '[uuid]',"
	. += "purpose =           [SQL_HELPER.Quote(purpose)],"
	. += "info =              [SQL_HELPER.Quote(info)],"
	. += "signer =            [SQL_HELPER.Quote(signer)],"
	. += "signer_account =    '[signer_account]',"
	. += "issuer =            [SQL_HELPER.Quote(issuer)],"
	. += "issuer_account =    '[issuer_account]',"
	. += "creator =           [SQL_HELPER.Quote(creator)],"
	. += "value =             [value],"
	. += "stocks =            [stocks],"
	. += "stocks_source_uid = [SQL_HELPER.Quote(stocks_source_uid)],"
	. += "state =             [state],"
	. += "contract_type =     [SQL_HELPER.Quote(contract_type)]"
