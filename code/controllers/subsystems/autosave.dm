...
/datum/controller/subsystem/autosave/stat_entry()
	...
	..(saving ? "Currently Saving" : "Next autosave in [round(time_diff/(1 MINUTE), 0.1)] minutes. (Every [config.save_interval MINUTES] minutes)")
...
/datum/controller/subsystem/autosave/proc/CalculateTimeNextSave()
	time_next_save = world.time + config.save_interval